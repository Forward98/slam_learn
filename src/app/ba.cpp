...
void prepareDataTUM(const std::string& dataFolder)
{
    ...
    while(!fin.eof())
    {
        // 从poses.txt读取3×4的位姿矩阵，将其转换为四元数和位置的格式，保存到pose_gt.txt
    ...
    // observations.txt 是一个包含观测数据的文件。
    // 这些观测数据描述了在不同帧中检测到的2D图像点及其对应的3D地标点索引
...
{
    // 实现了一个基于g2o库的非线性优化（图优化）来处理相机位姿和地标点的估计问题，
    // 主要功能是对位姿和地标点进行优化，提升估计精度。
    prepareDataTUM("../data");
