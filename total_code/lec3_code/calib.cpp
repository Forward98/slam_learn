...
void Calib::run()
{
    ...
    try
    {
        ...
        while(!fs.eof() && id < corner_pxs.rows())
        {
            ...
            // pose就是相机的位姿，要求解的就是这个和内参camera_K
            ...
            for(int i = 0; i < world_pts_.rows(); i++)
            {
                ...
                // 将求出的相机内参和外参应用到3D点投影到2D图像上, 投影后得到的像素坐标存储到reproject_pxs中
...
int main(int argc, char** argv)
{
    ...
    std::string conf_folder = "E:/VScode_project/slam_learn/conf/"; // 绝对路径
